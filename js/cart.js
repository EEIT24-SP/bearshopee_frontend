...
if (listObj.length == 0) { //购物车为空
	...
	for (var i = 0, len = listObj.length; i < len; i++) {
		...
		tr.innerHTML = '<td>' +
			...
			'</td>' +
			'<td>' +
			'<img class="cart-table-img" src="' + listObj[i].pImg + '" alt="" />' +
			'</td>' +
			'<td style="width:400px">' +
			listObj[i].pDesc +
			'</td>' +
			'<td>' +
			listObj[i].pDesc_type +
			'</td>' +
			'<td>' +
			'<button class="down">-</button><input style="width:30px" type="text" value="' + listObj[i].pCount + '" readonly="readonly" /><button class="up">+</button>' +
			'</td>' +
			'<td>' +
...
function getTotalPrice() {
	...
	for (var i = 0, len = cks.length; i < len; i++) {
		if (cks[i].checked) { //如果当前被选中
			...
			var temp = tr.children[6].firstElementChild.innerHTML;
			sum = Number(temp) + sum;
		}
	}
	return sum;
...
for (var i = 0, len = downs.length; i < len; i++) {
	downs[i].onclick = function () {
		...
		checkAllChecked();
		var price = tr.children[5].firstElementChild.innerHTML;
		tr.children[6].firstElementChild.innerHTML = price * txtObj.value;
		totalPrice.innerHTML = getTotalPrice();
